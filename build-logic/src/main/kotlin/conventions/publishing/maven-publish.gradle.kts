package conventions.publishing
...
val isReleaseVersion = provider { version.toString().matches(BuildLogicSettings.releaseVersionRegex) }
...
publishing {
   ...
   publications.withType<MavenPublication>().forEach {
      it.apply {
         ...
         pom {
            name.set("Context Logging")
            description.set("Facilities for logging using context receivers")
            url.set("https://github.com/Kantis/context-logging")

            scm {
               connection.set("scm:git:https://github.com/Kantis/context-logging/")
...
tasks.withType<AbstractPublishToMaven>().configureEach {
   ...
   val enabledPublicationNamePrefixes = settings.enabledPublicationNamePrefixes
